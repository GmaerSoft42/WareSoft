...
def setup():
    ...
    if setup_input == 'A' or setup_input =='This is Gloucester Road. Change here for the Circle line. This is a Piccadilly line service to Heathrow Terminals 2&3, and Terminal 5' :
        dbg = 'This is Gloucester Road. Change here for the Circle line. This is a Piccadilly line service to Heathrow Terminals 2&3, and Terminal 5'
        if setup_input == dbg:
            print("DEBUG Mode activated. Some features are disabled...")
        #a = input("Accept predefined partition mapping? This will erase all previous partition mappings. [Y, any other key to go to GKC FDISK]")
        #FDISK IS BROKEN!!!
        # if a != "Y":
        #     fdisk(0)W
        # else:
        #     partitions = {}
        partitions[0] = ["startable","installable","bootmgr-installed","GKCFS","C",524288,installed]
        # if 0 in partitions:
        #     if partitions[0][1] != 'installable':
        #         input("Setup cannot continue installing because Disk 1 Partition 0 is not installable. \nPress ENTER to restart setup, then check the partition mapping in FDISK")
        #         setup()
        #     if partitions[0][3] != "GKCFS":
        #         input("Setup cannot continue installing because Disk 1 Partition 0 does not have a recognised file system.\nPress ENTER to restart setup, then check the partition mapping in FDISK")
        #         setup()
        #     if partitions[0][4] != "C":
        #         input("Setup cannot continue installing because Disk 1 Partition 0 does not have a C: assignment.\nPress ENTER to restart setup, then check the partition mapping in FDISK")
        #         setup()
        # else:
        #     input("Setup cannot continue installing because Disk 1 has no partitions. .\nPress ENTER to restart setup, then check the partition mapping in FDISK")
        #     setup()
        ...
        for x in range(101):
            progressbar.append("░")
        if scandisk == '' and setup_input != dbg:
            for x in range(101):
    ...
    if setup_input == 'C':
        ...
        kickstart()
def fdisk(p):
    global partitions, partitioned, installable, startable, installed
    flag = True
    make_startable = False
    make_installable = False
    install_bootmgr = False
    make_main_part = False
    while flag == True:
        clear()
        print("FDISK")
        print('')
        print("Disk 1 (S-III 0): Integral Corp V Series PLUS 512GB SSD 524288MBytes")
        print(partitions)
        print('') 
        print("Options: ")
        print("1/ Create partition")
        print("2/ Delete partition")
        print("3/ Assign C: partition")
        print("4/ Set Installable")
        print("5/ Make Startable")
        print("6/ Install Boot Manager")
        print("7/ Format partition with GKCFS")
        print("8/ Unformat partition")
        print("9/ Exit")
        print("NOTE: GKC must be installed to Partition 0")
        option = input("Enter option: ")
        clear()
        if option == "1":
            print("Only one disk available - Partition will be created on Disk 1")
            size = int(input("Enter partition size in Mbytes (max 524288M bytes): "))
            if size < 524288:
                if len(partitions) > 2:
                    input("GKCFS only supports a maximum of 2 partitions per disk. To create a new partition, delete one or more partitions first. \n Press ENTER to return to main menu...")
                if 0 not in partitions:
                    partitions[0] = ["not-startable","not-installable","no-bootmgr","not-formatted","not-main-partition",size,installed]
                    flag = True
                if 1 not in partitions and flag != True:
                    if size < 524288 - partitions[0][5]:
                        partitions[1] = ["not-startable","not-installable","no-bootmgr","not-formatted","not-main-partition",size,installed]
                    else:
                        input("The partition that you created exceeds the maximum Mbyte value of 524288M. Please try with a lower partition size. Press ENTER to return to main menu...\nMaximum Size for partition 1: " + str({size < 524288 - partitions[0][5]}))
            else:
                input("The partition that you created exceeds the maximum Mbyte value of 524288M. Please try with a lower partition size. Press ENTER to return to main menu...")
        if option == "2":
            clear()
            if len(partitions) == 0:
                input("No partitions to delete. Press ENTER to return to main menu...")
            else:
                print(partitions)
                delete = int(input("Enter partition number to delete: "))
                if delete == 0 or delete == 1 and delete in partitions:
                    del(partitions[delete])
                    input("Deleted partition successfully. Press ENTER to return to main menu...")
                else:
                    input("Invalid partition specification. Press ENTER to return to main menu...")
        if option == "3":
            clear()
            if len(partitions) == 0:
                input("No partitions to set as C:. Press ENTER to return to main menu...")
            else:
                print(partitions)
                delete = int(input("Enter partition number to assign as C: "))
                if delete == 0 or delete == 1 and delete in partitions:
                    partitions[delete][4] = "C"
                    if delete == 0:
                        partitions[1][4] = "not-main-partition"
                    else:
                        partitions[0][4] = "not-main-partition"
                    input(f"Set partition {delete} as C: partition. Press ENTER to return to main menu...")
                    make_main_part = True
                else:
                    input("Invalid partition specification. Press ENTER to return to main menu...")    
        if option == "4":
            clear()
            if len(partitions) == 0:
                input("No partitions to set as Installable. Press ENTER to return to main menu...")
            else:
                print(partitions)
                delete = int(input("Enter partition number to set as Installable: "))
                if delete == 0 or delete == 1 and delete in partitions:
                    partitions[delete][1] = "installable"
                    if delete == 0 and 1 in partitions:
                        partitions[1][1] = "not-installable"
                    else:
                        partitions[0][1] = "not-installable"
                    input(f"Set partition {delete} as Installable. Press ENTER to return to main menu...")
                    make_installable = True
                else:
                    input("Invalid partition specification. Press ENTER to return to main menu...")    
        if option == "5":
            clear()
            if len(partitions) == 0:
                input("No partitions to set as startable. Press ENTER to return to main menu...")
            else:
                print(partitions)
                delete = int(input("Enter partition number to set as startable: "))
                if delete == 0 or delete == 1 and delete in partitions:
                    partitions[delete][0] = "startable"
                    if delete == 0:
                        partitions[1][0] = "not-startable"
                    else:
                        partitions[0][0] = "not-startable"
                    input(f"Set partition {delete} as startable. Press ENTER to return to main menu...")
                    make_startable = True
                else:
                    input("Invalid partition specification. Press ENTER to return to main menu...") 
        if option == "6":
            clear()
            if len(partitions) == 0:
                input("No partitions to install Boot Manager to. Press ENTER to return to main menu...")
            else:
                print(partitions)
                delete = int(input("Enter partition number to install Boot manager to: "))
                if delete == 0 or delete == 1 and delete in partitions:
                    partitions[delete][2] = "bootmgr-installed"
                    if delete == 0:
                        partitions[1][2] = "no-bootmgr"
                    else:
                        partitions[0][2] = "no-bootmgr"
                    input(f"Installed Boot Manager for partition {delete}. Press ENTER to return to main menu...")
                    install_bootmgr = True
                else:
                    input("Invalid partition specification. Press ENTER to return to main menu...")          
        if option == '7':
            clear()
            if len(partitions) == 0:
                input("No partitions to format. Press ENTER to return to main menu...")
            else:
                print(partitions)
                delete = int(input("Enter partition number to format: "))
                if delete == 0 or delete == 1 and delete in partitions:
                    if partitions[delete][3] != "GKCFS":
                        print("Partition of unknown type - Not saving UNFORMAT data")
                    partitions[delete][3] = "GKCFS"
                    if delete == 0:
                        partitions[1][3] = "GKCFS"
                    else:
                        partitions[0][3] = "GKCFS"
                    progressbar = []
                    print(f"Formatting partition {delete}, please wait...")
                    for x in range(101):
                        progressbar.append("░")
                    for x in range(101):
                            print(f'{x}% ', "".join(progressbar),end='\r')
                    ...
                    input(f"Formatted partition {delete}. Press ENTER to return to main menu...")
                else:
                    input("Invalid partition specification. Press ENTER to return to main menu...")      
        if option == '8':
            clear()
            if len(partitions) == 0:
                input("No partitions to unformat. Press ENTER to return to main menu...")
            else:
                print(partitions)
                delete = int(input("Enter partition number to unformat: "))
                if delete == 0 or delete == 1 and delete in partitions:
                    if partitions[delete][3] != "GKCFS":
                        input("UNFORMAT Data not saved - Operation can't be completed. Press ENTER to return to main menu...")
                        pass
                    print(f"Attempting to unformat partition {delete}, please wait...")
                    for x in range(101):
                        progressbar.append("░")
                    for x in range(38):
                            print(f'{x}% ', "".join(progressbar),end='\r')
                            progressbar[x] = "▓"
                            time.sleep(random.random())
                    input(f"The GKCFS File Table contains errors or changes to the disk have been made. Your disk has not been modified, UNFORMAT cannot revert the effects of FORMAT. Press ENTER to return to main menu...")
                else:
                    input("Invalid partition specification. Press ENTER to return to main menu...")  
        if option == '9':
            if p == 0:
                if make_startable != True:
                    print("WARNING!!! No partition is marked Startable.")
                    os.system('pause')
                if make_installable != True:
                    print("No partition with at least 50Mbytes was set as Installable")
                    os.system('pause')
                if make_main_part != True:
                    print("No partition was assigned as drive C:")
                    os.system('pause')
                if install_bootmgr != True:
                    print("Boot Manager was not installed. This may cause boot problems later")
                    os.system('pause')
            print("Closing FDISK...")
            time.sleep(1)
            flag = False
    ...
    try:
        try:
            while flag == True:
                ...
                print("[6] Copy FILES")
                print("[7] FDISK")
                print("[8] Enter CMD.EXE (Windows Only)")
                print("[9] Delete DIR")
            ...
            input("STOP : 0210\nYou do not have the permissions to use this file. Press ENTER to exit...")
            flag = False
            return
    ...
    try:
        ...
        try:
            ...
            while flag == True:
                ...
                if prompt == 'pwd':
                    pwd()
                if prompt == 'fdisk' or prompt == 'partman':
                    fdisk(1)
                if prompt == 'exit' or prompt == 'return':
                    ...
                    print("partman or fdisk : Enter FDISK partition manager")
...
def removable_device():
    ...
    if installed == False:
        ...
        if a == "Y":
            prodkey = winkey()
        oobe()
        # if partitions[0][0] == "startable":
        #     if partitions[0][2] == "bootmgr-installed":
        #         oobe()
        #     else:
        #         option = input("Boot Failure - Operating system was found but no boot manager was available.\n1/ Reboot Computer\n2/ Enter FDISK")
        #         if option == "2":
        #             fdisk(0)
        #         kickstart()
        # if partitions[1][0] == "startable":
        #     if partitions[1][2] == "bootmgr-installed":
        #         oobe()
        #     else:
        #         option = input("Boot Failure - Operating system was found but no boot manager was available.\n1/ Reboot Computer\n2/ Enter FDISK")
        #         if option == "2":
        ...
        #         kickstart()       
        # else:
        #         option = input("Boot Failure - No partition was set as startable\n1/ Reboot Computer\n2/ Enter FDISK")
        #         if option == "2":
        #             fdisk(0)
        #         kickstart()    
    else:
